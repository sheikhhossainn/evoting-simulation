...
const submitShareSchema = z.object({
  ...
  // (leading bits-tag byte + hex payload), never punctuation, spaces, or upper
  // case. z.string().min(1) accepted arbitrary garbage ("!!!!", "hello") and
  // only failed later at combine() time. Reject malformed shares at the schema
  // boundary so /keyshares/submit returns 400 instead of 201/500.
  share_value: z
    .string()

 ...
 * checkNullifier() and submitVote() now send the RAW NID — the backend
 * derives nid_hash, nullifier_hash, and constituency_code server-side
 * (see backend/src/crypto/identity.ts). The client no longer computes
...
/**
 * Check whether this voter has already voted in this election.
 * The backend computes the nullifier server-side from the raw NID —
 * the client never computes or even sees the nullifier.
 */
export async function checkNullifier(
  ...
  try {
    ...
    if (err instanceof TypeError) {
      ...
      const mockVotedNids = JSON.parse(localStorage.getItem("mock_voted_nids") || "[]");
      return { exists: mockVotedNids.includes(nid) };
    }
    throw err;
  }
}
...
export async function submitVote(
  ...
  try {
    ...
    if (err instanceof TypeError) {
      ...
      // Offline mock: remember this NID as having voted
      const mockVotedNids = JSON.parse(localStorage.getItem("mock_voted_nids") || "[]");

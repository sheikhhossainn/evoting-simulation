...
| **Network attacker** | On-path between browser ↔ backend ↔ Supabase ↔ chain | TLS assumed to hold; no certificate pinning yet (flagged in [FUTURE_WORK.md](../FUTURE_WORK.md)) |
...
| **Backend administrator / compromised backend** | Controls the Express process: env secrets (`NULLIFIER_SECRET`, `NID_HASH_SALT`, `ADMIN_SECRET`), can call any route including dev-tamper endpoints | **Currently over-trusted** for pre-anchor windows (§6); single shared `ADMIN_SECRET`, not per-admin (noted in [FUTURE_WORK.md](../FUTURE_WORK.md)) |
...
|---|---|---|
| Secrecy | ElGamal encryption client-side; nullifier has no reversible link to `nid`, salted + server-secret-keyed | **Fixed** — plaintext `candidate_id` removed from the wire entirely; the ZKP disjunction proof is now the sole mechanism establishing ballot validity |
...
| Uniqueness | Unique constraint on `nullifier_hash` + row lock in `fn_cast_vote` | Concurrency-tested — holds |
| Validity | Mandatory Chaum–Pedersen OR-proof, server-derived candidate set | Sound against a network/voter attacker. Not sound against a backend attacker who controls candidate-set derivation itself |
| Tamper-evidence | Merkle batch + on-chain anchor | **Deletion-completeness gap** (§5) and **pre-anchor window** (§6) — both open |
| Tally correctness | 3-of-4 partial decryptions, each accompanied by a Chaum–Pedersen DLEQ proof of correct computation from the submitter's share; backend verifies every proof before combining | **Closed** — a malicious keyholder submitting a wrong partial decryption is rejected at verification, not silently combined (§7) |
| Voter verifiability | None enforced today — Benaloh audit is design-only | Explicit boundary (§8), not silently gapped |
...
`fn_admin_delete_vote()` RPC (schema.sql), used solely by the admin-gated deletion-detection demo.

This gap is addressed by the Sparse Merkle Tree (`docs/smt-design.md`), anchored alongside the
dense tree at every batch: a real membership proof issued against an old anchored SMT root stays
valid forever, while a non-membership proof for the same key against a later root proves that key is
now absent — the contradiction between the two is what makes deletion of an already-anchored ballot
detectable (`GET /anchor/verify-smt/:voteId`, `POST /anchor/tamper/delete-vote` for the demo).

**Remaining gap, explicit per smt-design.md §8**:
...
Anchoring is batched, not per-vote ([anchoring-cost-analysis.md](./anchoring-cost-analysis.md)). Between
...
| Modified ciphertext post-insertion | Tamper-evidence | Covered — [tamper-proof-demo.md](./tamper-proof-demo.md) |
...
| Forged Merkle proof | Tamper-evidence | Covered — [system-overview-and-evaluation.md §5](./system-overview-and-evaluation.md) |
| Manipulated root | Tamper-evidence | Covered — [tamper-proof-demo.md](./tamper-proof-demo.md) |
| Insufficient Shamir shares | Tally correctness (confidentiality side) | Covered — [tally-verifiability-implementation-report.md](./tally-verifiability-implementation-report.md) |
...
| Concurrent double voting | Uniqueness | Covered — [system-overview-and-evaluation.md §3](./system-overview-and-evaluation.md) |

...
| T4 | Malicious/compromised admin action | Anyone with the shared `ADMIN_SECRET` can `POST /anchor/batch`, `/keyshares/tally`, and the **demo tamper routes** (`/anchor/tamper/*`, incl. `fn_admin_delete_vote`, which genuinely deletes a vote row) — no per-admin identity | A5 blocks casual deletes; A6/A7/A8 detection plus `services/adminAudit.ts` rows with the static C2 actor | **PARTIAL — attribution is auditable, per-admin identity remains out of scope** | (a) production-flag off the demo tamper routes; (b) B5 later: per-admin identity + sessions; (c) keep SMT + bundle checks |
...
| T13 | Voting after official close / close mid-submission | A stale or late client submits after the server leaves `voting` | Fresh `elections.status` read, forward-only admin transition, `isAcceptingVotes` gate, 403 `ELECTION_NOT_OPEN`, computed `availability` | **MITIGATED in code; live HTTP/race evidence pending `.env.test`** | Test DB transition/gate/race run; mobile reflects closed state (§3.T13) |
...
### §3.T13 — Enforced election open/close (Status: MITIGATED in code; live evidence pending)
1. **Threat:** ballots are accepted outside the official voting window; "closing" does not stop new votes.
2. **Scenario:** a late ballot reaches `POST /vote` after the fresh election row is no longer `voting`.
3. **Existing control:** P3's status CHECK, transition validator, append-only event audit, and shared `isAcceptingVotes` predicate. Status: **MITIGATED in code**.
4. **Control (server + DB):** (a) admin `PATCH /elections/:id/status` (admin secret now; B5 identity later) preserves the CHECK constraint and writes the status before its event row; (b) `/vote` rejects status ≠ `'voting'` with 403 `ELECTION_NOT_OPEN`; (c) registry responses expose computed availability. **Covers:** window integrity. **Doesn't:** a DB admin directly rewriting status.
...
4. **New control (server):** (i) expose the three demo tamper routes only when an explicit env flag is set (`ENABLE_TAMPER_DEMO=1`), so production doesn't ship them; (ii) record every guarded admin route through `services/adminAudit.ts` and the append-only `admin_actions` table (actor is static `shared-admin` under C2); (iii) replace the shared secret with per-admin credentials + server sessions later (B5; WebAuthn is the repo's own roadmap — FUTURE_WORK §11.4); (iv) keep SMT + bundle verification as the detection backstop. **Doesn't cover:** a DB admin running SQL directly — the repo's explicit boundary.

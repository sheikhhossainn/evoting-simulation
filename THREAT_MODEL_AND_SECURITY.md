...
- **Vote request shape post-migration:** `POST /vote` — `{election_id, encrypted_vote, zkp_proof}` + `Authorization`; the server derives `nid_hash`/`nullifier_hash`/`constituency_code` from the **session**, not from the body (A2 preserved; the raw NID no longer transits the wire on cast).

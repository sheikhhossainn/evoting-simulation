...
2. **There is no election-window check anywhere in the API.** `POST /vote` validates existence (404), key availability (503), and setup commitment (412), but never `elections.status` (AUDIT §15.4/§16.4). A mobile list ("Election Hub") would advertise elections that are not actually accepting votes. **Fix:** `PATCH /elections/:id/status` + status gate in `/vote` + `election_status_events` audit (T13/B2), and a computed `availability` field on `GET /elections` so the hub reflects the server's truth.

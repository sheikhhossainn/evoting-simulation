...
- **Tasks:** append DDL+tests-guarantees to `backend/src/schema.sql` (`sessions` — C1 CHECK fixed, `admin_actions`, `tally_runs`, `election_status_events` + guard triggers; extend `fn_votes_immutable_guard` to block `zkp_proof`); add error-envelope helper (`middleware/errorEnvelope.ts`); add a dependency-free fixed-window limiter + CAPTCHA hook (registration/vote tiers) — `express-rate-limit` was deliberately **not** adopted, so no new package and no backend lockfile change; add `ENABLE_TAMPER_DEMO` gating for the four demo routes (anchor.ts:340-662); add a `tally_runs` insert in `POST /keyshares/tally` (keyshares.ts:402-421) and switch `GET /public/results` to `tally_runs ORDER BY tallied_at DESC LIMIT 1` (public.ts:126) — **C3: no `tally_results` write or cache-sync is built**; `admin_actions` rows use the static actor `"shared-admin"` (C2).
- **Affected files:** `backend/src/schema.sql`, `backend/src/index.ts`, `backend/src/middleware/adminAuth.ts` (+ new `errorEnvelope.ts`, `rateLimit.ts`, `captcha.ts`, `tamperDemo.ts`, `middleware.test.ts`), `backend/src/routes/vote.ts`, `voter.ts`, `backend/src/routes/anchor.ts`, `keyshares.ts`, `public.ts`, `backend/.env.example`.
...
- **Status (P2): DONE** — sessions/`/voter/me`/refresh/revoke/revoke-all, `/candidates` bearer-first (with `?constituency=` removed), and `/vote` session-first identity via `services/castIdentity.ts`. The schema gained `sessions.nullifier_hash` (decision A, BUILD_NOTES §7) so the session path casts the same pseudonym as the raw-NID path. The A1 invariant is asserted in `castIdentity.test.ts`; the lifecycle suite is `sessionStore.test.ts` (both in `test:ci`, no database). Not yet executed against a live DB (Open Question #11).
